...
window.chat = (appContext, currentUser, allChatHTML, messageSentHTML, messageReceivedHTML) => {
    const dbRef = ref(database);
    ...
    var unsubscribeOnChildAdded = null

    // get or create user
    myUserId = getOrCreateUser(myUserId, currentUser.username, currentUser.avatar);
    ...
    const onLoadMessage = (chatId, myUserId) => {
        ...
        unsubscribeOnChildAdded = onChildAdded(child(dbRef, `messages/${chatId}`), (data) => {
            get(child(dbRef, `/users/${data.val().senderId}`)).then(u => {
                let user = u.val()

                if (data.val().senderId === myUserId) {
                    messagesHTML = messageSentHTML(data.val().message, data.val().timestamp, user.avatar)
                } else {
                    messagesHTML =  messageReceivedHTML(data.val().message, data.val().timestamp, user.avatar)
                }
    ...
    // load chats
//     let chatsHTMLItem = (chatId, name, avatar, active, lastMessage, timestamp, numberNewMessage) => {
//         return `<li class="p-1 border-bottom" id="${chatId}">
...
//     }
    ...
    onChildAdded(child(dbRef, `userRooms/${myUserId}`), (chat) => {
        if (chat.exists()) {
            get(child(dbRef, `members/${chat.key}`)).then((users) => {
                let otherUserId = Object.values(users.val()).filter(value => value !== myUserId)[0]
                ...
                get(child(dbRef, `users/${otherUserId}`)).then(otherUser => {
                    ...
                    get(child(dbRef, `chats/${chat.key}`)).then(chatObject => {
                        ...
                        chatsHTML = allChatHTML(chat.key, user.name, user.avatar, true,
                            chatObj.lastMessage, new Date(chatObj.timestamp).toLocaleString(),
                            1)
    ...
    var addChat = (myUserId, user) => {
        ...
        // load all members
        chatId = null;
        get(child(dbRef, "members")).then(data => {
            return data.val()
        }).then((allMembers) => {
            ...
            if (allMembers !== null) {
                for (let member in allMembers) {
                    if (Object.values(allMembers[member]).indexOf(myUserId) > -1
            ...
            if (chatId === null) {
                ...
                set(child(dbRef, `members/${chatId}`), {userId1: myUserId, userId2: user.id});
    ...
    $("#kw-user").keyup(function () {
        if (document.getElementById("kw-user").value === '') {
            ...
            fetch(`${appContext}api/users-chat/?kw-username=${document.getElementById("kw-user").value}`, {
                method: "GET", headers: {
                    "Content-Type": "application/json"
                }
                                ...
                                <a href="javascript:;" class="d-flex justify-content-between">
                                    <div class="d-flex flex-row">
                                        ...
                                        <div class="pt-0">
                                            ...
                                            <p class="mb-0 text-white" style="font-size: 12px;"> Bùi Khánh Huy</p>
                                        </div>
                                    </div>
                                </a>
                            </li>`
