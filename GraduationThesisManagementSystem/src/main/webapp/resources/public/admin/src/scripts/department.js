const showAddDepartmentModal = (endpoint) => {
    document.getElementById("myModalAddAndEditDepartment").innerText = "Thêm khoa"
...
const showEditDepartmentModal = (endpoint, departmentId) => {
    loadDepartmentById(endpoint, (data) => {
        document.getElementById("myModalAddAndEditDepartment").innerText = "Cập nhật khoa"
        document.getElementById("btn-submit-form").onclick = () => saveChange(endpoint, departmentId)
        $('#modal-add-edit-department').modal()

        for (let d in data) {
            document.forms['form-add-edit-department'][d].value = data[d];
            if (d === "founding") {
                document.forms['form-add-edit-department'][d].value = new Date(data[d]).toISOString().slice(0, 10);
            } else {
                document.forms['form-add-edit-department'][d].value = data[d];
            }
        }
...
const showViewDepartmentModal = (endpoint) => {
    loadDepartmentById(endpoint, (data) => {
        for (let d in data) {
            if (d === 'founding') {
                document.getElementById(`data-${d}`).innerText = new Date(data[d]).toISOString().slice(0, 10)
            } else {
    ...
    if (departmentId === null) {
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                ...
                successfulAlert("Thêm khoa thành công", "Ok", () => location.reload());
            } else {
                // errror
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                // successful
                $('#modal-add-edit-department').hide();
                successfulAlert("Cập nhật thông tin khoa thành công", "Ok", () => location.reload());
            } else {
                // errror
                $.each(data, function (key, value) {
                    $('input[name=' + key + ']').after('<span class="text-danger">' + value + '</span>');
