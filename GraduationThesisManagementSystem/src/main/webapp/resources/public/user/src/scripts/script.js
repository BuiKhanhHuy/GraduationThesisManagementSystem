...
const changePasswordByUser = (appContext, userId) => {
    document.getElementById("btn-submit-form-change-password").onclick = () => saveChangePassword(appContext, userId);
...
const saveChangePassword = (appContext, userId) => {
    ...
    fetch(`${appContext}api/users/password`, {
        method: "POST", headers: {
            ...
            "userId": userId, "oldPassword": formData.oldPassword, "newPassword": formData.newPassword
        })
        ...
        if (Object.keys(data).length === 0) {
            // successful
            modal.hide();
            successfulAlert("Thay đổi mật khẩu thành công. \nBạn cần đăng nhập lại!", "Ok", () => document.getElementById("logoutForm").submit());
        } else {
            // error
...
const uploadReportFile = async (appContext, thesisId) => {
    Swal.fire({
        ...
        preConfirm: () => {
            var reportFile = document.forms['form-upload-file']['reportFile'].files[0];
            ...
            if (fileName === '') {
                ...
                if (reportFile) {
                    ...
                    formData.append("fileName", new Blob([JSON.stringify(fileName.toString())], {
                    ...
                    return fetch(`${appContext}api/theses/${thesisId}/report-file`, {
                        ...
                        .then(response => {
                            if (!response.ok) {
                                errorAlert("Đã có lỗi", "Upload tập tin khóa luận không thành công!", "Ok")
                            } else {
                                successfulAlert("Upload tập tin khóa luận thành công.", "Ok", () => location.reload());
                            }
