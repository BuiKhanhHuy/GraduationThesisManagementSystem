const showAddSchoolYear = (endpoint) => {
    document.getElementById("myModalAddAndEditSchoolYear").innerText = "Thêm niên khóa"
...
const showEditSchoolYear = (endpoint, schoolYearId) => {
    loadSchoolYearById(endpoint, (data) => {
        console.log(data)
        document.getElementById("myModalAddAndEditSchoolYear").innerText = "Cập nhật niên khóa"
        document.getElementById("btn-submit-form").onclick = () => saveChange(endpoint, schoolYearId)
        $('#modal-add-edit-school-year').modal()

        for (let d in data) {
            document.forms['form-add-edit-school-year'][d].value = data[d];
            if (d === "startDate" || d === "endDate") {
                document.forms['form-add-edit-school-year'][d].value = new Date(data[d]).toISOString().slice(0, 10);
            } else {
                document.forms['form-add-edit-school-year'][d].value = data[d];
            }
        }
    ...
    if (schoolYearId === null) {
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                ...
                successfulAlert("Thêm niên khóa thành công", "Ok", () => location.reload());
            } else {
                // errror
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                // successful
                $('#modal-add-edit-school-year').hide();
                successfulAlert("Cập nhật thông tin niên khóa thành công", "Ok", () => location.reload());
            } else {
                // errror
                $.each(data, function (key, value) {
                    $('input[name=' + key + ']').after('<span class="text-danger">' + value + '</span>');
