const showAddPosition = (endpoint) => {
    document.getElementById("myModalAddAndEditPosition").innerText = "Thêm chức vụ"
...
const showEditPosition = (endpoint, positionId) => {
    loadPositionById(endpoint, (data) => {
        document.getElementById("myModalAddAndEditPosition").innerText = "Cập nhật chức vụ"
        document.getElementById("btn-submit-form").onclick = () => saveChange(endpoint, positionId)
        $('#modal-add-edit-position').modal()
    ...
    if (positionId === null) {
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                ...
                successfulAlert("Thêm chức vụ thành công", "Ok", () => location.reload());
            } else {
                // errror
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                // successful
                $('#modal-add-edit-position').hide();
                successfulAlert("Cập nhật thông tin chức vụ thành công", "Ok", () => location.reload());
            } else {
                // errror
                $.each(data, function (key, value) {
                    $('input[name=' + key + ']').after('<span class="text-danger">' + value + '</span>');
...
const deletePositionItem = (endpoint) => {
    ...
    confirmAlert("Bạn có chắc không?", "Bạn sẽ không thể khôi phục điều này!", "Có, xóa nó", "Không, hủy bỏ", () => {
      fetch(endpoint, {
            ...
            if (res.status === 204) successfulAlert("Xóa chức vụ thành công", "Ok", () => location.reload());
        }).catch(err => {
