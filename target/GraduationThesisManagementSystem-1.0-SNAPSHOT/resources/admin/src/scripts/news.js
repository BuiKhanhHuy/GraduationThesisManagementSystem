...
const showEditNewsModal = (endpoint, newsId) => {
    loadNewsById(endpoint, (data) => {
        document.getElementById("myModalAddAndEditNews").innerText = "Cập nhật bản tin"
        document.getElementById("btn-submit-form").onclick = () => saveChange(endpoint.replace("/single", ""), newsId)
        $('#modal-add-edit-news').modal()
    ...
    if (newsId === null) {
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                ...
                successfulAlert("Thêm bản tin thành công", "Ok", () => location.reload());
            } else {
                console.log(data)
                // error
                $.each(data, function (key, value) {
                    if (key !== "content")
                        ...
                        document.getElementById("error-content").innerText = value
                });
            }
        }).catch(err => {
        ...
        fetch(endpoint, {
            ...
            if (Object.keys(data).length === 0) {
                // successful
                $('#modal-add-edit-news').hide();
                successfulAlert("Cập nhật thông tin bản tin thành công", "Ok", () => location.reload());
            } else {
                // error
                $.each(data, function (key, value) {
                    if (key !== "content")
